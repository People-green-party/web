                        ...
                        return (
                            <ScrollReveal key={i} animation="fade-up" delay={i * 100}>
                                <div className="group relative h-full">
                                    ...
                                    <div className="relative bg-white border border-[#0D5229]/10 p-10 rounded-[40px] h-full flex flex-col items-center text-center shadow-sm hover:shadow-xl transition-all duration-500">
                                        <div className={`w-20 h-20 rounded-2xl flex items-center justify-center mb-8 transition-colors duration-700
                                            ${isPGPActive ? 'bg-[#10B981]/10 text-[#10B981]' : 'bg-[#EF4444]/10 text-[#EF4444]'}`}>
                                            <item.icon size={36} strokeWidth={1.5} />
                                        </div>

                                        <h3 className="text-[#04330B] text-2xl font-bold mb-2 tracking-tight">{categoryName}</h3>
                                        <p className={`font-bold text-xs uppercase tracking-widest mb-10 transition-colors duration-700
                                            ...
                                            {item.stats.map((stat, j) => (
                                                <div key={j} className="flex flex-col gap-3">
                                                    <div className="flex justify-between items-end text-sm">
                                                        <span className="text-[#587E67] font-bold text-[10px] uppercase tracking-wider">{stat.label}</span>
                                                        <span className="font-bold text-2xl" style={{ color: stat.color }}>{stat.value}</span>
                                                    </div>
                                                    <div className="h-1.5 w-full bg-[#F8FBF9] rounded-full overflow-hidden">
                                                        <div
                                        ...
                                        <div className="mt-12">
                                            {isPGPActive ? (
                                                <CheckCircle2 size={32} strokeWidth={1.5} className="text-[#10B981] animate-bounce" />
                                            ) : (
                                                <AlertTriangle size={32} strokeWidth={1.5} className="text-[#EF4444] animate-pulse" />
                                            )}
                                        </div>
                                    </div>
                                </div>
